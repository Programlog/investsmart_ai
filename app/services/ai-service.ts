...
const systemInstruction = `You are a helpful AI chatbot specialized in finance and the economy. Your primary focus is to discuss topics such as investment strategies, market trends, and financial planning. 
  ...
  Keep your responses concise and helpful within the scope of finance and basic greetings/farewells. Important: keep your responses under 100 words (your input token limit is 400), emojis are acceptable.`
...
export async function generateText(prompt: string, onChunk?: (chunk: string) => void, userContext?: any): Promise<string> {
  let fullPrompt = prompt;
  if (userContext) {
    let contextString = '';
    if (userContext.questionnaireResponses && userContext.questionnaireResponses.length > 0) {
      contextString += 'User questionnaire responses:\n';
      for (const resp of userContext.questionnaireResponses) {
        contextString += `- ${resp.questionId}: ${resp.answer}\n`;
      }
    }
    if (userContext.watchlist && userContext.watchlist.length > 0) {
      contextString += 'User watchlist:\n';
      for (const stock of userContext.watchlist) {
        contextString += `- ${stock.symbol}`;
        if (stock.notes) contextString += ` (notes: ${stock.notes})`;
        contextString += `\n`;
      }
    }
    if (contextString) {
